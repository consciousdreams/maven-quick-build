...
public class MavenActionsRegistrar implements AppLifecycleListener {

    static final String PREFIX = "it.consciousdreams.dynamic.";
    ...
    /**
     * Registers/unregisters DynamicMavenActions in ActionManager to match current settings.
     * Safe to call multiple times (idempotent for unchanged actions).
     */
    static void sync() {
        ActionManager am = ActionManager.getInstance();
        List<MavenActionConfig> configs = MavenQuickBuildSettings.getInstance().getActions();
        ...
        for (MavenActionConfig config : configs) {
            String id = PREFIX + config.id;
            AnAction existing = am.getAction(id);
            if (existing instanceof DynamicMavenAction dma && dma.getConfig().equals(config)) {
            ...
            am.registerAction(id, new DynamicMavenAction(config));
            registeredIds.add(id);
            applyShortcut(keymap, id, config.shortcut);
        }
    }

    private static void applyShortcut(Keymap keymap, String actionId, @org.jetbrains.annotations.Nullable String shortcut) {
